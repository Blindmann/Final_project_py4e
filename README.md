...
- In the **Yr_no** folder can be found the main content of my final project, where I:
    ...
    2) used the **second** program _yrno_modified.py_ in order to use these retrieved latitude longitude values for each weather stations and to predict the weather at these locations (using the [LocationForecast API](https://developer.yr.no/doc/locationforecast/HowTO/)). Both the parser and the yrno_modified.py program use the same **_SQLite3 database_** that they contribute on building (the yrno_modified.py program adds a table to the **weatherstations.sqlite database** in order to save the forecast data).
